    ...
    useEffect(() => {
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                transitionLogo.style.cssText = 'position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); z-index:10000; pointer-events:none; opacity:0; display:flex; flex-direction:column; align-items:center; gap:25px; transition: color 0.1s;';

                transitionLogo.innerHTML = `
                    <img src="/assets/logo-removebg-preview.png" class="loading-main-logo" alt="Bharat Buildathon" style="width: 250px; height: auto;" />
                    <div style="display: flex; flex-direction: column; align-items: center; gap: 10px;">
                        <span style="font-size: 0.9rem; font-family: 'DM Sans', sans-serif; font-weight: 700; text-transform: uppercase; letter-spacing: 3px; opacity: 0.8;">Hosted By</span>
                        <div style="display: flex; align-items: center; gap: 20px;">
                            <img src="/assets/Alexa Developers Community Logo.png" alt="Alexa" style="height: 35px; width: auto;" class="loading-partner-logo" />
                            <span style="font-size: 1.2rem; font-weight: 700;">×</span>
                            <img src="/assets/GFG_logo_dark_bg[1].png" alt="GFG" style="height: 35px; width: auto;" class="loading-partner-logo" />
                        </div>
                    </div>
                `;
            ...
            const initialCover = document.querySelector('.initial-load-cover');
            const isInitial = !!initialCover;
            ...
            if (isInitial) {
                // Initial Load: Scribble is already fully drawn over the screen
                gsap.set(transitionScribblePath, { strokeDasharray: l, strokeDashoffset: 0, strokeWidth: config.strokeWidthMax, opacity: 1 });
                gsap.set(transitionScribbleSvg, { scale: config.scale, opacity: 1, x: 0, y: 0, rotation: 0 });
                gsap.set(transitionLogo, { opacity: 1, scale: 1 });
                
                gsap.to(transitionLogo.querySelector('.loading-main-logo'), { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                
                // Instantly swap the solid cover for the perfectly matching SVG
                gsap.set(initialCover, { display: 'none' });
                initialCover.remove();
                
                // Draw out after a short reading delay
                const delay = 1.0;
                drawTl.to(transitionScribblePath, { strokeDashoffset: -l, duration: durOut, ease: 'power2.inOut' }, delay);
                drawTl.to(transitionScribblePath, { strokeWidth: config.strokeWidthStart, duration: durOut, ease: 'power2.inOut' }, delay);
                
                drawTl.to(transitionLogo, {
                    autoAlpha: 0,
                    onComplete: () => {
                        gsap.killTweensOf(transitionLogo.querySelector('.loading-main-logo'));
                        gsap.set(transitionLogo.querySelector('.loading-main-logo'), { rotation: 0 });
                    }
                }, delay + (durOut * 0.48));
                
            } else {
                // Normal click transition (Draw in, then draw out)
